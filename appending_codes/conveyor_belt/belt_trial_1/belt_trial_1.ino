...
void handleSerialInput() {
  ...
  Serial.println("Enter position (0 -255):");

  while (!validInput) {
    if (Serial.available() >= 1) {  // Wait for 1 byte (0–255)
      int byteValue = Serial.read(); // Read the single byte (0–255)
      input = byteValue / 255.0;     // Convert to 0.0–1.0
      Serial.println(input);
      // Validate (technically redundant but kept for safety)
      if (input >= 0.0 && input <= 1.0) {
        validInput = true;
      } else {
        Serial.print("Corrupted data: ");
        Serial.print(input, 3);
        Serial.println("! Resend value (0-255)");
      }
    }
    leftSwitch.loop();
    rightSwitch.loop();
